    ...
    <nav className="group fixed top-0 w-full bg-accent shadow-lg z-50 p-2">
      <div className="flex justify-between">
        ...
        <div className="flex justify-end">
          {account === SELLER_ADDRESS ? <div className='ml-3'><button className='bg-secondary p-2 font-bold text-background rounded-full group-hover:cursor-pointer' onClick={handleApproval}>Approval</button></div> : null}
          {account === INSPECTOR_ADDRESS || account === LENDER_ADDRESS || account === SELLER_ADDRESS ? <div className='ml-3'><button className='bg-secondary p-2 font-bold text-background rounded-full group-hover:cursor-pointer' onClick={handleAddProperty}>Add Property</button></div> : null}
